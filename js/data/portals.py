...
def portal_generator(n_doors, n_floors):
    ...
    floors_n_doors = {}
    n_doorsperfloor = math.ceil(n_doors / n_floors) # round up to nearest integer
    # assign doors to each floor
    doors_list = list(range(1, n_doors+1))
    for floor in range(1, n_floors+1): # 1 thru 4 floors floorsndoors[4] = range 3*3+1, 10
...
def portal_solver(portal_connections, floors_n_doors):
...
def portal_picker(n_doors, n_floors, steps_to_solve):
    steps = 0
    reps = 0
    
    while steps != steps_to_solve:
        reps += 1
        assignments = portal_generator(n_doors, n_floors) # generate new set of portal connections
        portal_connections = assignments[0]
        floors_n_doors = assignments[1]
        steps = portal_solver(portal_connections, floors_n_doors) # solve for this set of portal connections

        if steps == steps_to_solve: # if it matches, return this set of portal connections
            break
        if reps >= 100:
            print("No solution after trying more than 100 times")
            return
